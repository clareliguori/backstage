...
describe('DefaultAwsCredentialsProvider', () => {
  ...
  describe('#getCredentials', () => {
    it('retrieves assume-role creds for the given account ID', async () => {
      const provider = await DefaultAwsCredentialsProvider.fromConfig(config);
    ...
    it('retrieves static creds from the main account', async () => {
      ...
      const provider = await DefaultAwsCredentialsProvider.fromConfig(
        minConfig,
      );
      const awsCredentials = await provider.getCredentials({
    ...
    it('rejects account that does not have the default role', async () => {
      stsMock
        .on(GetCallerIdentityCommand)
        .resolvesOnce({
          Account: '123456789012',
      ...
      const provider = await DefaultAwsCredentialsProvider.fromConfig(config);
      await expect(
        provider.getCredentials({ accountId: '999999999999' }),
      ).rejects.toThrow(/Invalid credentials/);
    });
  });

  describe('#fromConfig', () => {
    it('rejects account that is configured with credentials for a different account', async () => {
      stsMock
        .on(GetCallerIdentityCommand)
        .resolvesOnce({
          Account: '123456789012',
      ...
      await expect(
        DefaultAwsCredentialsProvider.fromConfig(config),
      ).rejects.toThrow(
        /The credentials provided for 222222222222 are for a different account/,
      );
    });

    it('rejects main account that has invalid credentials', async () => {
      stsMock.on(GetCallerIdentityCommand).rejects('Invalid credentials');
      await expect(
        DefaultAwsCredentialsProvider.fromConfig(config),
      ).rejects.toThrow(/Invalid credentials/);
    });

    it('rejects account that has invalid credentials', async () => {
      stsMock
        .on(GetCallerIdentityCommand)
        .resolvesOnce({
          Account: '123456789012',
